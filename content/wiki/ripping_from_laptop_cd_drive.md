...
These rip well using `CDDA_DEVICE=/dev/cdrom cdda2mp3 CD_WHATEVER`.  Make a directory to rip into, cd there, then run that.  On Debian `cdda2mp3` is part of the `icedax` package and also needs `lame` as an encoder.
